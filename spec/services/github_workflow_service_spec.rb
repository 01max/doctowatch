...
RSpec.describe GithubWorkflowService do
  ...
  describe '#enable' do
    it 'issues a PUT to the workflow enable endpoint with auth headers' do
      stub = stub_request(:put, 'https://api.github.com/repos/owner/repo/actions/workflows/check.yml/enable')
             .with(headers: headers).to_return(status: 204)

      service.enable('check.yml')
